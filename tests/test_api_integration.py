...
try:
    ...
    if stations_with_timestamps:
        ...
        for i, (pfs, fuel_price) in enumerate(stations_with_timestamps[:3], 1):
            ...
            if fuel_price.price_last_updated:
                age = (
                    datetime.now(fuel_price.price_last_updated.tzinfo)
                    - fuel_price.price_last_updated
                )
...
try:
    ...
    wool_stations = [
        s
        for d, s in results
        if "wool" in s.trading_name.lower() or "bovington" in s.trading_name.lower()
    ]
            ...
            for pfs in pfs_list:
                if pfs.node_id == station.node_id:
                    ...
                    for fp in pfs.fuel_prices:
                        if fp.price:
                            print(
                                f"      {fp.fuel_type}: {fp.price}p (updated: {fp.price_last_updated})"
                            )
                    break
    else:
